...
struct WishlistController: RouteCollection {
    ...
    func update(req: Request) async throws -> Summary {
        ...
        try await ActivityService.notifyRecipients(wishlistID: wishlistID, actorID: userId, kind: "wishlist_updated", title: "Shared wishlist updated", message: "\(actorName) renamed “\(title)”. Tap to check it out.", on: req.db, client: req.client, logger: req.logger)
