...
3. To enable website sign-in, register a Service ID (for Hushful, `com.bausch.hushful.web`) grouped with the primary App ID. Register `www.hushful-app.com` and `https://www.hushful-app.com` as its web domain and return URL, and set `APPLE_WEB_SIGN_IN_CLIENT_ID=com.bausch.hushful.web` on the API deployment. This popup-based flow verifies Apple’s signed identity token directly and does not expose a private key to the website.
4. Test on a real device with a separate Apple Account: create an account with both Share My Email and Hide My Email, cancel the sheet, retry a sign-in, and confirm that replaying the same identity token is rejected. Also test a returning account, which may not receive a name or email again.
5. If Hushful sends email to users who choose Hide My Email, register the sending domain and every From address with Apple's Private Email Relay configuration. Keep the approved sender aligned with `EMAIL_VERIFICATION_FROM_EMAIL` and `PASSWORD_RESET_FROM_EMAIL`.
